...
int main(int argc, char* argv[]) {
	char buf[BUF_LEN], buf1[BUF_LEN], buf2[BUF_LEN], buf3[BUF_LEN], whisper_to[BUF_LEN];
	int i, j, n, ret;
	...
	while (1) {
		...
		for (i = 0; i < num_chat; i++) {
			...
			if (FD_ISSET(client_fds[i], &read_fds)) {
				...
				sscanf(buf, "%s", buf1); // 처음 문자열 분리 [username] 
				...
				if (strncmp(buf2, CHAT_CMD_TO, strlen(CHAT_CMD_TO)) == 0) {
					memset(whisper_to, 0, BUF_LEN);
					memset(buf3, 0, BUF_LEN);
					sscanf(buf2 + strlen(CHAT_CMD_TO) + 1, "%s", whisper_to);
					sprintf(buf3, "%s", buf2 + strlen(CHAT_CMD_TO) + 1 + strlen(whisper_to) + 1);

					memset(buf, 0, BUF_LEN);
					sprintf(buf, "[귓속말] [%s] %s", userlist[i], buf3);
					for (j = 0; j < num_chat; j++) {
						if ((strncmp(userlist[j], whisper_to, strlen(whisper_to)) == 0) && usersleep[j] != 1) {
							if (send(client_fds[j], buf, BUF_LEN, 0) <= 0) {
								printf("whisper sending error for client[%d]\n", j);
								client_error[j] = 1;
							}
						}
					}
					printf("[귓속말] from [%s] to %s : %s", userlist[i], whisper_to, buf3);
					continue;
				}
				...
				// /sleep 처리
				// [user1] /sleep
				if (strncmp(buf2, CHAT_CMD_SLEEP, strlen(CHAT_CMD_SLEEP)) == 0) {
					usersleep[i] = 1;
					continue;
				}
				...
				// /wakeup 처리
				// [user1] /wakeup
				if (strncmp(buf2, CHAT_CMD_WAKEUP, strlen(CHAT_CMD_WAKEUP)) == 0) {
					usersleep[i] = 0;
					continue;
				}
