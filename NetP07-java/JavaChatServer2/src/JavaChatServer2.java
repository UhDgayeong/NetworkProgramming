...
public class JavaChatServer2 extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					JavaChatServer2 frame = new JavaChatServer2();
		...
		public void run() {
			while (true) { // 사용자 접속을 계속해서 받기 위해 while문
				try {
					...
					String line = new String(b, "euc-kr");
					line = line.trim(); // 앞뒤 blank NULL, \n 모두 제거
					
					String[] msg = line.split(" ");
					// [user1] /list --> msg[0] msg[1] ..
					AppendText(line); // server 화면에 출력
					
					if (msg.length == 1) {
						continue;
					}
					if (msg[1].matches("/list")) {
						// /list 처리
						WriteOne("User list\n" + "Name\tStatus\t\n" 
						+ "----------------------------------\n");
						for (int i=0; i<user_vc.size(); i++) {
						...
						for (int j=0; j<user_vc.size(); j++) {
							...
							if (user.UserName.matches(msg[2]) && user.UserStatus == "0") {
								user.WriteOne("[귓속말] " + msg[0] + m + "\n");
							}
						}
						continue;
						
					}
					else if (msg[1].matches("/exit")) {
						// /exit 처리
						WriteAll(UserName + "님이 퇴장하였습니다.\n");
					...
					else if (msg[1].matches("/sleep")) {
						// /sleep 처리
						this.UserStatus = "S";
						continue;
					}
					else if (msg[1].matches("/wakeup")) {
						// /wakeup 처리
						this.UserStatus = "0";
						continue;
					}
